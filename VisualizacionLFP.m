...
fprintf('\n');
pause(3) % 3 seg. para observar los canales y sus areas
...
for m = 1:length(ia)%1:largo_dataAll  
    ...
    for q = 1:largo_areasActuales
        ...
        %plot(time_step_m(time_step_m<tiempo_total), -(500*q)+data_all(:,areas_actuales(q)));
        plot(time_step_m(time_step_m<tiempo_total), -(500*q)+REGISTRO.channel(canales_eval(areas_actuales(q))).data);
        hold on;
    ...
    for q = 1:length(areas_actuales)
        ...
        histo_diff = histogram(diff(REGISTRO.channel(canales_eval(areas_actuales(q))).data),200);
