function [z,mu,sigma,amp_max] = zscore_noartifacted(x,ind_over_threshold,flag,dim)
% Edited - Aquiles
...
x_dearifacted = x((~ind_over_threshold));
mu = mean(x_dearifacted,dim);
sigma = std(x_dearifacted,flag,dim);
sigma0 = sigma;
sigma0(sigma0==0) = 1;
amp_max = max(x_dearifacted) - mu;
%z = x;
