...
elseif strcmp(registroLFP.reference_type, 'area') %% Referencia al promedio de cada area (ex M1L)
    ...
    for m = 1:length(ia) 
        ...
        avarage_area = mean(data_artifacted_area,2);
        ...
        for j = 1:largo_area_actual 
            data_ref_artifacted = registroLFP.channel(canales_eval(areas_actuales(j))).data_raw - avarage_area;
            registroLFP.channel(canales_eval(areas_actuales(j))).data_ref = data_ref_artifacted; 
